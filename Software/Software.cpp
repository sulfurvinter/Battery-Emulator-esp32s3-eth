...
#include "src/devboard/ethernet/comm_ethernet.h"
#include "src/communication/nvm/comm_nvm.h"
#endif

#if !defined(HW_LILYGO) && !defined(HW_LILYGO2CAN) && !defined(HW_STARK) && !defined(HW_3LB) && !defined(HW_BECOM) && \
    !defined(HW_WAVESHARE) && !defined(HW_DEVKIT) && !defined(HW_ESP32S3_GENERIC)
...
void init_serial() {
  // Init Serial monitor
  Serial.begin(115200);
#if (HW_LILYGO2CAN || HW_BECOM || HW_WAVESHARE || HW_ESP32S3_GENERIC)
...
// 2026-06-04: Added by Claude — hold BOOT button (GPIO0) for 5s at startup to reset network settings
#ifdef HW_ESP32S3_GENERIC
static void check_network_reset_button() {
  if (digitalRead(0) != LOW) {
    return;  // Button not held, nothing to do
  }
  logging.println("BOOT button held — hold 5 seconds to reset network settings...");
  unsigned long start = millis();
  while (digitalRead(0) == LOW) {
    if (millis() - start >= 5000) {
      logging.println("Resetting network settings to defaults...");
      BatteryEmulatorSettingsStore settings(false);
      settings.saveString("SSID",          "");
      settings.saveString("PASSWORD",      "");
      settings.saveString("APNAME",        "BatteryEmulator");
      settings.saveString("APPASSWORD",    "123456789");
      settings.saveBool("WIFIAPENABLED",   true);
      settings.saveBool("ETHMODE",         false);
      settings.saveBool("STATICIP",        false);
      settings.saveBool("WEBAUTH",         false);
      settings.saveString("HTTPUSER",      "admin");
      settings.saveString("HTTPPASS",      "");
      logging.println("Network settings reset. Rebooting...");
      delay(500);
      esp_restart();
    }
    delay(50);
  }
  logging.println("BOOT button released early — network reset cancelled.");
}
#endif
...
#ifdef HW_ESP32S3_GENERIC
  check_network_reset_button();
  if (ethernet_enabled) {
    init_ethernet();
  }
#endif

  // Init wifi (always — provides AP fallback even when Ethernet is active)
  init_WiFi();
