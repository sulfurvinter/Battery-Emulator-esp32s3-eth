...
void init_webserver() {
  ...
  const char* boolSettingNames[] = {
      ...
      "LOWPASSFILTER", "WEBAUTH",       "SLOWCANINV",    "UPDCHECK",
  ...
  // 2026-06-04: Added by Claude — signed int settings for ESP32-S3 generic board pin assignments (GPIO_NUM_NC = -1)
#ifdef HW_ESP32S3_GENERIC
  const char* intSettingNames[] = {
      "PIN_CAN_TX",  "PIN_CAN_RX",   "PIN_RS485_TX",  "PIN_RS485_RX",  "PIN_RS485_DE",
      "PIN_MCP_SCK", "PIN_MCP_MOSI", "PIN_MCP_MISO",  "PIN_MCP_CS",    "PIN_MCP_INT",
      "PIN_MCPFD_SCK","PIN_MCPFD_SDI","PIN_MCPFD_SDO", "PIN_MCPFD_CS",  "PIN_MCPFD_INT",
      "PIN_MCPFD2_SCK","PIN_MCPFD2_SDI","PIN_MCPFD2_SDO","PIN_MCPFD2_CS","PIN_MCPFD2_INT",
      "PIN_POS_CNT", "PIN_NEG_CNT",  "PIN_PRECH",     "PIN_BMS_PWR",   "PIN_LED",
      "PIN_DISP_SDA","PIN_DISP_SCL",
      "PIN_ETH_SCK", "PIN_ETH_MOSI", "PIN_ETH_MISO", "PIN_ETH_CS", "PIN_ETH_INT", "PIN_ETH_RST",
      "ETH_SPI_BUS",
  };

  // Handles the form POST from UI to save settings of the common image
  server.on("/saveSettings", HTTP_POST,
            [boolSettingNames, stringSettingNames, uintSettingNames, intSettingNames](AsyncWebServerRequest* request) {
#else
  // Handles the form POST from UI to save settings of the common image
  server.on("/saveSettings", HTTP_POST,
...
#ifdef HW_ESP32S3_GENERIC
                for (auto& intSetting : intSettingNames) {
                  if (p->name() == intSetting) {
                    auto value = atoi(p->value().c_str());
                    settings.saveInt(intSetting, value);
                  }
                }
#endif
...
// Show hardware used:
    ...
    // 2026-07-04: Modified by Claude — update check is opt-in (UPDCHECK setting, default off)
